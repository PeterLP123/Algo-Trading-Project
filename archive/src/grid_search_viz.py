...
Produces:
  1. Heatmap grid of wf_score by DEAD_ZONE x rebalance_every.
  2. Horizontal ranking bar chart.
  3. Mean validation Sharpe vs mean turnover scatter for every grid point.
"""
...
def plot_grid_search(wf_search_df, thresh_grid, rebal_grid, output_dir):
    """Plot heatmaps, ranking bar chart, and Sharpe-vs-turnover scatter."""
        ...
        for rebal in rebal_grid:
            ...
            fig_hm.add_trace(
                go.Heatmap(
                    ...
                    colorbar={"title": {"text": "wf_score"}, "len": 0.8} if plot_idx == 1 else None,
    ...
    bar_colors = [
        config.COLORS["cost"] if bool(selected) else config.COLORS["net"]
        for selected in wf_sorted["is_selected"]
    ]
    fig_bar = go.Figure()
    fig_bar.add_trace(
        go.Bar(
    ...
    scatter_df = wf_search_df.copy()
    scatter_colors = np.where(scatter_df["is_selected"], config.COLORS["cost"], config.COLORS["benchmark"])
    scatter_symbols = np.where(scatter_df["is_candidate"], "diamond", "circle")
    fig_scatter = go.Figure()
    fig_scatter.add_trace(
        go.Scatter(
            x=scatter_df["mean_val_turnover"],
            y=scatter_df["mean_val_sharpe"],
            mode="markers+text",
            text=[
                f"{int(r['MA_WINDOW'])}|{int(r['VOL_WINDOW'])}|{r['DEAD_ZONE']}|{int(r['rebalance_every'])}"
            ...
            marker={
                ...
                "line": {"width": 1, "color": config.COLORS["close"]},
            },
            hovertemplate=(
                "MA=%{text}<br>mean_val_turnover=%{x:.2f}<br>mean_val_sharpe=%{y:.3f}"
                "<br>candidate=%{customdata[0]}<br>selected=%{customdata[1]}<extra></extra>"
            ),
            customdata=np.column_stack(
                [scatter_df["is_candidate"].astype(str), scatter_df["is_selected"].astype(str)]
            ),
            name="Grid point",
        )
    )
    fig_scatter.update_layout(
        title="Validation Sharpe vs Mean Turnover",
        xaxis_title="Mean validation turnover",
        yaxis_title="Mean validation Sharpe",
        width=1200,
        height=700,
    )
    save_plotly_fig(fig_scatter, "wf_validation_sharpe_vs_turnover", output_dir)
