"""Trend strategy utilities for the walk-forward notebook."""
...
def apply_rebalance_theta(
    ...
    # Use a writable array because Arrow/CoW-backed blocks can be read-only.
...
def mvo_weights(
    ...
    """Solve the mean-variance problem and return zero weights on failure."""
    n = len(mu)
    w = cp.Variable(n)
    ...
    *,
    ...
    if cached_features is None:
        ...
        for s in symbols:
            ...
            # z is used for activation and diagnostics only.
    ...
    if use_mvo:
        if feature_cache is not None and cache_key in feature_cache:
            ...
            # Daily returns panel used for covariance estimation.
        ...
        if search_fast_mode:
            ...
            for t_idx in range(len(close_px)):
                z_t = z.iloc[t_idx].to_numpy(dtype=float)
                active_t = active.iloc[t_idx].to_numpy(dtype=bool)
                # Zero inactive assets so the dead-zone threshold is respected.
                mu_t = np.where(active_t, z_t, 0.0)
                n_active = int(active_t.sum())

                if t_idx < cov_window:
                    # Not enough history yet: use equal-weight fallback.
                    w_row = (
                        np.where(active_t, np.sign(z_t) / n_active, 0.0)
                    ...
                    if len(cov_slice) < min_hist:
                        # Too many missing rows: use equal-weight fallback.
                        w_row = (
                            np.where(active_t, np.sign(z_t) / n_active, 0.0)
                            if n_active > 0 else np.zeros(n_assets)
                        )
        ...
        # Original inverse-volatility path.
        # The dead-zone applies to z; volatility is applied once in w_risk.
